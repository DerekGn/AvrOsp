			...
			case 'Y' : //RC Calibrate (for AVR057)
                 ...
                 if( strlen( param ) <= 2 )
					cout<<"Flash Address not specified! OSCCAL value will not be written!\r\n";
						...
						try
						{
							OSCCAL_FlashAddress_Tiny = convertHex( param + 2 );
						}
						catch( ErrorMsg * e )
						{
							delete e;
							throw new ErrorMsg( "Cannot convert hex number for -Y parameter!" );
						}
                 break;
...
void JobInfo::doJob()
{
	...
	/* RC Calibration for AVR057 */
    if( RC_Calibrate )
    {
        Util.log( "Sending Calibration Clock...\r\n");
        calibRetval = prog->RC_Calibrate();
        if( !calibRetval )
            throw new ErrorMsg( "Calibration Failed!" );
    }
...
void JobInfo::doDeviceDependent( AVRProgrammer * prog, AVRDevice * avr )
{
	...
	if( programFlash || verifyFlash )
	{
		...
		if(RC_Calibrate)
		{
            if(OSCCAL_FlashAddress_Tiny>flashStartAddress && OSCCAL_FlashAddress_Tiny<flashEndAddress)
               throw new ErrorMsg("Specified address is within application code! OSCCAL will not be written!\r\n");
            else
            {
                hex->setData( OSCCAL_FlashAddress_Tiny,(long)calibRetval);
                hex->setUsedRange( flashStartAddress, OSCCAL_FlashAddress_Tiny );
            }          
        }
	}
